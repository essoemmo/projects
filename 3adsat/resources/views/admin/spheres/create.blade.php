...
'title' => _i('Add Sphere'),
'subtitle' => _i('Add Sphere'),
'activePageName' => _i('Add Sphere'),
...
'form-horizontal']) }} --}}
<form class="form-horizontal" action="{{ route('spheres.store') }}" method="post" enctype="multipart/form-data" id="addForm" role="form" data-toggle="validator">
    @csrf
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h5 class="card-title">{{ _i('Add Sphere') }}</h5>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                                        ...
                                        <div class="card-body">
                                            <div class="tab-content">
                                                <div class="tab-pane active" id="tab-general">
                                                    ...
                                                    <div id="show_sphere" style="display: none;">
                                                    ...
                                                    <div id="show_sphere_no_price" style="display: none;">
                                                        <div class="form-group">
                                                            <label for="title" class="col-sm-2 control-label">{{ _i('Title') }}<span style="color: #F00;">*</span></label>
                                                            <div class="col-sm-10">
...
<script>
    ...
    $(".show_options").on("change",function(){
        ...
        if(show_options == 1) {
            $("#show_sphere").show();
            $('#show_sphere_no_price').find('#title').removeAttr('required');
            $('#show_sphere_no_price').find('#price').removeAttr('required');
        } else {
            $("#show_sphere").hide();
            $('#show_sphere_no_price').find('#title').prop('required',true);
            $('#show_sphere_no_price').find('#price').prop('required',true);
        }
        if(show_options == 2) {
            $("#show_sphere_no_price").show();
            $('#show_sphere').find('#title').removeAttr('required');
            $('#show_sphere').find('#price').removeAttr('required');
        } else {
            $("#show_sphere_no_price").hide();
            $('#show_sphere').find('#title').prop('required',true);
            $('#show_sphere').find('#price').prop('required',true);
        }
    });
</script>
