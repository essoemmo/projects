...
<a class="btn btn-danger btn-sm" data-toggle="modal" href="{{route('front_users.destroy', $id) }}" data-id="{{$id}}" data-target="#custom-width-modal"><i class="ti-trash"></i> {{_i('delete')}}</a>


<form action="{{route('front_users.destroy', $id) }}" method="post" class="remove-record-model">
    {{csrf_field()}}
    {{method_field('delete')}}
    @csrf
