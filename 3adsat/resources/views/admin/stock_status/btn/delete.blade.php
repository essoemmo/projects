...
<a href="#" data-target=".edit_modal" data-toggle="modal" class="btn btn-success edit" data-id="{{$id}}"
   @foreach($languages as $lang) data-title-{{$lang->code}}="{{\App\Models\StockStatusDescription::where('stock_status_id',$id)->where('language_id',$lang->id)->first()['name']}}" @endforeach>
    <i class="ti-pencil-alt"></i></a>


<a class="btn btn-danger waves-effect waves-light remove-record" data-toggle="modal" data-url="{{\Illuminate\Support\Facades\URL::route('stockStatus.destroy', $id)}}" data-id="{{$id}}" data-target="#custom-width-modal"><i class="ti-trash" style="color: #fff;"></i></a>
