        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table">
                    ...
                    <tr class="productRow" v-for="pro, index in SavedProducts" :key="pro.id"
                        :class="{hidden: productvisible}">
                        <td>
                            <button style="float: right;margin-top: 20px;margin-left: 20px;" class="btn btn-danger"
                                @click="productdel(index)"><span>x</span></button>
                            <div class="media">
                    ...
                    <tr>
                        ...
                        <td>{{shippingOption != null && shippingOption != 0 ? shippingOption.cost+Number(shippingOption.cash_delivery_commission) : 0}}
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>{{shippingOption != null && shippingOption != 0 ? shippingOption.cost+Number(shippingOption.cash_delivery_commission)+sallaTotal : sallaTotal}}
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-3">
                                <a href="javascript:void(0)" @click="addProduct()"><i class="fa fa-plus"></i> إضافة منتج
                                    جديد</a>
                            </div>
                            <div class="col-md-8 col-md-offset-1">
                                <input @keyup="getsearch(product.title)" type="text" class="form-control"
                                    v-model="product.title" placeholder="البحث في قائمة المنتجات">
                                    
                                <ul :class="{ hidden: menuvisible}" class="list-unstyled search-menu">
                                    <li v-for="product, index in productsfilter" @click="getproduct(index)"
                                        :key="product.id" :style="{opacity: product.product.max_count == 0 ? 0.4 : 1}">
                                        <div class="media">
                                            <img v-if="product.product.product_photos[0]"
                                                class="media-object media-right img-responsive"
                                                :src="product.product.product_photos[0].photo">
                                            <img v-else class="media-object media-right img-responsive"
                                                src="/images/placeholder.png">
                                            <div class="media-body media-left">
                                                <div>{{product.title}}</div>
                                                <div>{{product.product.price}}</div>
                                            </div>
                                            <span style="color:red" v-if="product.product.max_count == 0">تم اجتياز الحد
                                                الاقصي للمنتج</span>
                                        </div>
                                    </li>
                                </ul>
                                    ...
                                    <div class="col-md-8">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input name="title" required="" class="form-control"
                                                    v-model="product.title" placeholder="اسم المنتج">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="productType"></label>
                                                <select name="productType" required="" class="selectpicker"
                                                    v-model="product.type">
                                                    <option v-for="type in product_type" :key="type.id"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input name="count" required="" class="form-control"
                                                    placeholder="الكميه" v-model="product.max_count">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="price"></label>
                                                <input name="price" required="" class="form-control" placeholder="السعر"
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <div v-if="product.features" v-for="option, index in product.features">
                                        <label :for="option.id">
                                            <input type="radio" v-model="selectedFeature" :name="option.title"
                                                :value="option" @change="selectfeature">
                                        ...
                                        <table class="table">
                                            ...
                                            <tr v-for="opt in option.options" v-if="opt.count > 0">
                                                <td>
                                                    <label :for="opt.id">
                                                        <input type="radio"
                                                            :disabled="selectedFeature == null || selectedFeature.id != opt.feature_id"
                                                            @change="selectOption" v-model="selectedOptionFeature"
                                                            :name="opt.title" :value="opt">
                                                        {{opt.title}}
                                                    </label>
                                                </td>
                                                <td>
        ...
        methods: {
            ...
            productdel(index) {
                this.allProducts = this.allProducts.map((pro) => {
                    if (pro.id == this.SavedProducts[index].id) {
                        pro.product.max_count = this.SavedProducts[index].max_count + this.SavedProducts[index]
                            .count;
                    }
                    return pro;
                });
            ...
            countchange(count, index) {
                if (this.SavedProducts[index].max_count + this.SavedProducts[index].count < count) {
                    ...
                    if (minuscount < 0) {
                        this.SavedProducts[index].max_count = (this.SavedProducts[index].max_count + this.SavedProducts[
                            index].count) - Number(count);
                        this.SavedProducts[index].count = Number(count);
                    }
                    this.allProducts = this.allProducts.map((pro) => {
                        if (pro.id == this.SavedProducts[index].id) {
                    ...
                    if (this.SavedProducts[index].option != null) {
                        this.sallaTotal += minuscount * (this.SavedProducts[index].price + this.SavedProducts[index]
                            .option.price);
                    } else {
