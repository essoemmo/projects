...
class PagesDataTable extends DataTable
{
        ...
        return datatables($query)
            ...
            ->addColumn('edit', function($query){
                $html = '<a href="#" data-toggle="modal" data-target="#get_link" data-link="'.url('pages/'.$query->id).'" class="btn btn-default get_link"  title="' . _i("Show Link") . '">
                   <i class="ti-link"></i></a>  &nbsp;'.'';
                $html .= '<a href="'. url('adminpanel/pages/'.$query->id.'/edit') .'" id="item_id_' . $query->id . '" class="btn btn-primary"  title="' . _i("Edit") . '">
                   <i class="ti-pencil-alt"></i></a>  &nbsp;'.'
                   <form class=" delete"  action="'.url("adminpanel/pages/".$query->id) .'"  method="POST" id="deleteRow"  
    ...
    {
        $query = Page::leftJoin('pages_data' ,'pages_data.page_id','pages.id')
            ->select('pages.*','pages_data.page_id','pages_data.lang_id','pages_data.source_id',
                'pages_data.title','pages_data.content')->where('pages_data.source_id' , null);
            ...
            ->parameters([
                ...
                'buttons' => [
                    [
                        ...
                        window.location = "'.url('adminpanel/pages/create').'";}',
                    ],
