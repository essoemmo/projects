...
class PermissionController extends Controller
{
    ...
    public function  getDatatablePermission()
    {
        ...
        $permission = Permission::select(['id', 'name' ,'created_at', 'updated_at'])->where('guard_name' ,$guard_store);
...
//            'name' =>  array('required','regex:/^[\p{L}\s-]+$/u', 'max:255', 'unique:permissions') // rules to accept string only
        ...
        if($validator->fails()){
            ...
            $role = Role::where('name','super-store')->first();
            if(!$role)
            {
                // Create a super-admin role for the admin users
                $role = Role::create(['guard_name' => $guard_store , 'name' => 'super-store']);
                $role->save();
                $user->assignRole($role->name); // attached  user with role
            }
            $permission = Permission::create(['name' => $request->name , 'guard_name' => $guard_store]); // create permission
            $permission->save();
            $role->givePermissionTo($permission->name);  // attached role with permission that have same guard name
            $admin_role = Role::where('name','super-store')->first();
...
//        dd(Admin::where('id','=',Auth::guard('admin')->user()->id)->first()->hasPermissionTo('Add-Permission', 'admin'));
    ...
    public function updatePermission(Request $request)
    {
        ...
        return redirect('/adminpanel/permission/all')->with('success' ,_i('Updated Successfully !'));
    }
    ...
    public function deletePermission($id)
    {
        ...
        return redirect('/adminpanel/permission/all')
