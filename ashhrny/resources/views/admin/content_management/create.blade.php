...
@section('content')
    <div class="card">
        ...
        <div class=" card-block">
            <form action="{{route('content_management.store')}}" method="post" class="j-forms" id="j-forms"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" name="lang_id">
                                ...
                                @foreach($langs as $lang)
                                    <option
                                        value="<?=$lang['id']?>" <?=old('lang_id') == $lang['id'] ? 'selected' : ''?> ><?=_i($lang['title'])?></option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-control" name="type">
                                <option selected disabled><?=_i('CHOOSE')?></option>
                                <option
                                    value="home" <?=old('type') == 'home' ? 'selected' : ''?> ><?=_i('Home')?></option>
                                <option
                                    value="footer" <?=old('type') == 'footer' ? 'selected' : ''?> ><?=_i('Footer')?></option>
                            </select>
                        </div>
                       ...
                       <div class="col-md-1"></div>
                        ...
                        <div class="col-md-7">
                            ...
                            <select class="form-control" name="columns" required="" id="column_select">
                                ...
                                @for($i = 1 ; $i <= 4 ; $i++)
                                    <option
                                        value="<?=$i?>" <?=old('columns') == $i ? 'selected' : ''?> ><?=$i?></option>
                                @endfor
                            </select>
                        </div>
                    </div>
                                ...
                                @foreach($langs as $key => $lang)
                                    <div class="tab-pane @if ($loop->first) active @endif" id="{{ $lang->locale }}"
                                        ...
                                        <div class="form-group row">
                                            <label class="col-md-2 control-label ">{{ $lang->title }} <?=_i('Title')?>
                                                <span style="color: #F00;">*</span></label>
                                            <div class="col-md-7">
                                                <input id="title" class="form-control " name="{{ $lang->locale }}_title"
                                                       data-parsley-maxlength="191" value="<?=old('title')?>"
                                                       required="">
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <label>{{ $lang->title }} {{_i('Column1')}} <span
                                                        style="color: #F00;">*</span></label>
                                                <textarea class="textarea form-control ckeditor"
                                                          name="{{ $lang->locale }}_content[]"
                                                          data-parsley-minlength="10"
                                                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                          placeholder="{{ $lang->title }} {{ _i('content column1 here...') }} ">{{old('content[0]')}}</textarea>
                                            </div>

                                            <div class="col-md-6 column2" style="display:none">
                                                <label>{{ $lang->title }} {{_i('Column2')}} <span
                                                        style="color: #F00;">*</span></label>
                                                <textarea class="textarea form-control ckeditor"
                                                          name="{{ $lang->locale }}_content[]"
                                                          data-parsley-minlength="10"
                                                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                          placeholder="{{ $lang->title }} {{ _i('content column2 here...') }}">{{old('content[1]')}}</textarea>
                                            </div>

                                            <div class="col-md-6 column3" style="display:none">
                                                <label>{{ $lang->title }} {{_i('Column3')}} <span
                                                        style="color: #F00;">*</span></label>
                                                <textarea class="textarea form-control ckeditor"
                                                          name="{{ $lang->locale }}_content[]"
                                                          data-parsley-minlength="10"
                                                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                          placeholder="{{ $lang->title }} {{ _i('content column3 here...') }}">{{old('content[2]')}}</textarea>
                                            </div>

                                            <div class="col-md-6 column4" style="display:none">
                                                <label>{{ $lang->title }} {{_i('Column4')}} <span
                                                        style="color: #F00;">*</span></label>
                                                <textarea class="textarea form-control ckeditor"
                                                          name="{{ $lang->locale }}_content[]"
                                                          data-parsley-minlength="10"
                                                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                          placeholder="{{ $lang->title }} {{ _i('content column4 here...') }}">{{old('content[3]')}}</textarea>
                                            </div>
                                        </div>
