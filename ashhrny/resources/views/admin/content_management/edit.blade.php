...
@section('content')
    ...
    <!-- =====Filter Section===== -->
    <div class="box-body">
        <form action="{{route('content_management.update',$content_section->id)}}" method="POST"
                    ...
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="{{route('content_management.index')}}"
                                                               class="btn btn-default"><i
                                            class="ti-list"></i> {{ _i('All Contents') }}</a></li>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control" name="lang_id">
                                        ...
                                        @foreach($langs as $key => $lang)
                                            <option
                                                value="<?=$lang['id']?>" @if(!empty($content_data[0])) {{$content_data[0]['lang_id'] == $lang['id'] ? 'selected' : ''}} @endif ><?=_i($lang['title'])?></option>
                                        @endforeach
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control" name="type">
                                        <option selected disabled><?=_i('CHOOSE')?></option>
                                        <option
                                            value="home" <?=$content_section['type'] == 'home' ? 'selected' : ''?> ><?=_i('Home')?></option>
                                        <option
                                            value="footer" <?=$content_section['type'] == 'footer' ? 'selected' : ''?> ><?=_i('Footer')?></option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4  {{ $errors->has('order') ? ' has-error' : '' }}">
                                    ...
                                    <select class="form-control" name="order" required="">
                                        ...
                                        @for($i = 1 ; $i <= 10 ; $i++)
                                            <option
                                                value="<?=$i?>" <?=$content_section['order'] == $i ? 'selected' : ''?> ><?=$i?></option>
                                        @endfor
                                    </select>
                                    @if ($errors->has('order'))
...
-->
                                ...
                                <div class="col-md-7">
                                    ...
                                    <select class="form-control" name="columns" required="" id="column_select">
                                        ...
                                        @for($i = 1 ; $i <= 4 ; $i++)
                                            <option
                                                value="<?=$i?>" <?=$content_section['columns'] == $i ? 'selected' : ''?> ><?=$i?></option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <div class="tab-content card-block">
                                        @foreach($langs as $key => $lang)
                                            <div class="tab-pane @if ($loop->first) active @endif"
                                                 id="{{ $lang->locale }}" role="tabpanel" aria-expanded="false">

                                                <div class="form-group row">
                                                    <label
                                                        class="col-md-2 control-label ">{{ $lang->title }} <?=_i('Title')?>
                                                        <span style="color: #F00;">*</span></label>
                                                    <div class="col-md-7">
                                                        <input id="title" class="form-control "
                                                               name="{{ $lang->locale }}_title"
                                                               data-parsley-maxlength="191"
                                                               value="<?=$content_data_title->where('locale', $lang->locale)->first()->title?>"
                                                               required="">
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row">
                                                    <div
                                                        style="display: none;!important;"> <?= $contents = \App\Models\ContentSectionTranslation::where('content_section_id', $content_section['id'])->where('locale', $lang->locale)->get(); ?></div>

                                                    @foreach($contents  as $k => $single)

                                                        <div class="col-md-6 column{{$k+1}}">
                                                            <label>{{_i('Column')}}{{$k+1}} <span
                                                                    style="color: #F00;">*</span></label>
                                                            <textarea class="textarea form-control ckeditor"
                                                                      name="{{$lang->locale}}_content[]"
                                                                      data-parsley-minlength="10"
                                                                      style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                                      placeholder="Place write content here..."><?=$single['content']?></textarea>
                                                        </div>
                                                    @endforeach
                                                    ...
                                                    @if(empty($content_data[0]))
                                                        ...
                                                        <div class="col-md-6 column1">
                                                            ...
                                                            <textarea class="textarea form-control ckeditor"
                                                                      ...
                                                                      data-parsley-minlength="10"
                                                                      style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"
                                                                      placeholder="Place write content here..."></textarea>
                                                        </div>
