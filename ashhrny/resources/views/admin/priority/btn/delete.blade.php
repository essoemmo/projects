...
<a href="priority/{{$id}}/edit" data-target=".edit_modal" data-toggle="modal" class="btn btn-success edit" data-id="{{$id}}"
   @foreach(langs() as $lang)
        data-title-{{$lang->locale}}="{{\App\Models\PriorityTranslation::where('priority_id',$id)->where('locale',$lang->locale)->first()['title']}}"
    @endforeach>
    <i class="ti-pencil-alt"></i></a>


<a class="btn btn-danger waves-effect waves-light remove-record" data-toggle="modal" data-url="{{ \Illuminate\Support\Facades\URL::route('priority.destroy', $id) }}" data-id="{{$id}}" data-target="#custom-width-modal"><i class="ti-trash" style="color: #fff;"></i></a>
