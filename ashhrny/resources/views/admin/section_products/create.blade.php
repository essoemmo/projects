...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-block">
            <form role="form" action="{{route('section_products.store')}}" class="j-forms" method="post" id="fileupload"
...
-->
                    ...
                    @foreach($langs as $lang)
                        <div class="form-group">
                            ...
                            <input type="text" name="{{ $lang->locale }}_title" id="title"
                                   value="{{ old($lang->locale.'_title') }}" class="form-control" required
                                   data-validate="true">
                    ...
                    <div class="form-group">
                        <label class="col-xs-2 exampleInputEmail1" for="order">
                            {{ _i(' Order ') }} </label>
                        <select class="form-control" style="width: 100%;" id="order" name="order" required
                                data-validate="true">
                            <option disabled selected>{{ _i('Select') }}</option>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <select class="js-example-basic-multiple form-control" name="product_id[]"
                                            data-tags="true" data-placeholder="{{ _i('Select a Product') }}"
                                            multiple="multiple" required="">
                                        @foreach($products as $key =>  $item)
                                            <option
                                                value="{{ $key }}" {{ ( old("product_id") == $key ? "selected":"") }} >{{ $item }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
