...
?>
@if(admin()->user()->can($item["permission"]))
    <li class="{{$active}} <?=(isset($item['type']) && $item["type"] == 'item') ? 'pcoded-hasmenu' : 'pcoded'?> ">
        <a href="<?= (isset($item['type']) && $item["type"] == 'none') ? url($item["route"]) : 'javascript:void(0)' ?>"
           class="">
            ...
            @if(!empty($item["count"]))
                <span class="pcoded-badge label label-danger"> {{$item["count"]}}</span>
            @endif
        ...
        <ul class="pcoded-submenu ">
            ...
            ?>
            @if(admin()->user()->can($sub["permission"]))
                <li class=" {{$active}} <?=(isset($item['type']) && $item["type"] == 'item') ? "" : 'pcoded'?>">
                    <a href="{{url($sub['route'])}}">
                        ...
                        @if(!empty($sub["count"]))
                            <span class="pcoded-badge label label-danger"> {{$sub["count"]}}</span>
                        @endif
                        <span class="pcoded-mcaret"></span>
...
<nav class="pcoded-navbar" pcoded-header-position="relative">
    ...
    <div class="pcoded-inner-navbar main-menu">
        ...
        <div class="pcoded-navigatio-lavel" data-i18n="nav.category.navigation"
             menu-title-theme="theme5">{{ _i('Navigation') }}</div>
