...
@section('content')
    ...
    <div class="wrap">
        <section class="app-content">
            ...
            <h3>تعديل  الصلاحيلات</h3>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item"><a href="#">الصلاحيات</a></li>
                <li class="breadcrumb-item active" aria-current="page">تعديل الصلاحيلات</li>
            </ul>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="widget p-lg">
                        <form action="{{route('roles.update',$role->id)}}" method="post" style="padding: 30px">
                            {{csrf_field()}}
                            {{method_field('put')}}
                                                ...
                                                <td>
                                                    ...
                                                    <select class="form-control select2" name="permissions[]" multiple>
                                                        @foreach($maps as $index=>$map)
                                                            <option
                                                                value="{{$map.'_'.$model}}" {{$role->hasPermission($map.'_'.$model) ? 'selected':''}}>{{$map}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                            </tr>
