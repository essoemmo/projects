...
//                    $file = asset(str_replace(" ", "%20", $row->files));

                    $btn = '<a   class="btn btn-success btn-sm print" data-file="'.$path.'" data-id="'.$row->id.'"><i class="fa fa-download"></i></a>';
                    $btn .= '<a   class="btn btn-info btn-sm sendEmail" data-file="'.$row->files.'" data-id="'.$row->id.'" data-toggle="modal" data-target="#exampleModal" style="margin-right: 2px"><i class="fa fa-send"></i></a>';
...
//    dd($fileName);
    ...
    public function sendEmail(Request $request){
        ...
        Mail::send('admin.reports.attach', $dataa, function($message) use ($request) {
            $name = strstr($request->email,'@',true);
            $message->to($request->email, $name)->subject
            ('You recived file');
