...
{{--                                <span>{{$cityname}}</span>--}}
                                ...
                                <ul class="list-inline single-member-options">
                                    ...
                                    @if(\Illuminate\Support\Facades\Auth::check())
                                        ...
                                        @if(!empty($fav) && $fav->action == 'like' && $fav->from_id == auth()->user()->id )
                                            <li><a href="javascript:void(0)"  class="add-to-fav-online" id="{{'like-'.$usernat->id}}" data-id="{{$usernat->id}}"  data-to="{{$usernat->id}}" data-from="{{\Illuminate\Support\Facades\Auth::id() ? \Illuminate\Support\Facades\Auth::id() : ''}}" title="اضافه لقائمة الاهتمام"><i class="fa fa-heart"></i></a></li>
                                        @else
                                            <li><a href="javascript:void(0)"  class="add-to-fav-online" id="{{'like-'.$usernat->id}}" data-id="{{$usernat->id}}" data-to="{{$usernat->id}}" data-from="{{\Illuminate\Support\Facades\Auth::id() ? \Illuminate\Support\Facades\Auth::id() : ''}}"><i class="fa fa-heart-o"></i></a></li>
                                        @endif

                                    @else
                                        @if(!empty($fav) && $fav->action == 'like')
                                            <li><a href="javascript:void(0)"  class="add-to-fav-online" id="{{'like-'.$usernat->id}}" data-id="{{$usernat->id}}"  data-to="{{$usernat->id}}" data-from="{{\Illuminate\Support\Facades\Auth::id() ? \Illuminate\Support\Facades\Auth::id() : ''}}" title="اضافه لقائمة الاهتمام"><i class="fa fa-heart-o"></i></a></li>
                                        @else
                                            <li><a href="javascript:void(0)"  class="add-to-fav-online" id="{{'like-'.$usernat->id}}" data-id="{{$usernat->id}}" data-to="{{$usernat->id}}" data-from="{{\Illuminate\Support\Facades\Auth::id() ? \Illuminate\Support\Facades\Auth::id() : ''}}"><i class="fa fa-heart-o"></i></a></li>
                                        @endif
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center">
                ...
                {{$onlineUser->appends(request()->query())->links()}}

            </ul>
        </nav>
    @else
        <div class="alert alert-danger">
            <h4>{{_i('Sorry dont Found onlineuser')}}</h4>
        </div>
    @endif
