...
// (such as highlight rows, day rows, Help rows, etc).
...
Grid.mixin({
			...
			cellOver: function(cell, isOrig) {
				...
				if (dropLocation) {
					if (view.renderDrag(dropLocation, seg)) { // have the subclass render a visual indication
						mouseFollower.hide(); // if the subclass is already using a mock event "Help", hide our own
					}
					else {
						mouseFollower.show();
					}
					...
					// have the Help follow the mouse (no snapping) with a warning-style cursor
					mouseFollower.show();
					disableCursor();
				}
			},
	...
	// A truthy returned value indicates this method has rendered a Help element.
...
var DayGrid = Grid.extend({
	...
	helperEls: null, // set of cell skeleton elements for rendering the mock event "Help"
	...
	renderDrag: function(dropLocation, seg) {
		...
		// if a segment from the same calendar but another component is being dragged, render a Help event
		if (seg && !seg.el.closest(this.el).length) {
			...
			return true; // a Help has been rendered
		}
	},
	...
	// Renders a mock "Help" event. `sourceSeg` is the associated internal segment object. It can be null.
	renderHelper: function(event, sourceSeg) {
		var helperNodes = [];
	...
	// Unrenders any visual indication of a mock Help event
	destroyHelper: function() {
		if (this.helperEls) {
...
var TimeGrid = Grid.extend({
	...
	helperEl: null, // cell skeleton element for rendering the mock event "Help"
	...
	// A returned value of `true` signals that a mock "Help" event has been rendered.
	renderDrag: function(dropLocation, seg) {
		var opacity;

		if (seg) { // if there is event information for this drag, render a Help event
			this.renderRangeHelper(dropLocation, seg);
			...
			return true; // signal that a Help has been rendered
		}
		else {
	...
	// Renders a mock "Help" event. `sourceSeg` is the original segment object and might be null (an external drag)
	renderHelper: function(event, sourceSeg) {
		var segs = this.eventsToSegs([ event ]);
		var tableEl;
		var i, seg;
...
var BasicView = fcViews.basic = View.extend({
	...
	// Affects Help-skeleton and highlight-skeleton rows.
	introHtml: function() {
		if (this.weekNumbersVisible) {
...
fcViews.agenda = View.extend({ // AgendaView
	...
	// Affects content-skeleton, Help-skeleton, highlight-skeleton for both the time-grid and day-grid.
